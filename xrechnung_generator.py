...
# Typ 381 (Gutschrift/Storno) muss in UBL als eigenes Dokument CreditNote
# ausgegeben werden — ein ubl:Invoice mit InvoiceTypeCode 381 lehnt KoSIT ab
# (BR-CL-01). Gleiche Kindelemente, andere Wurzel + drei Tagnamen (03.09.2026).
NS_CN = "urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2"
NSMAP_CN = {None: NS_CN, "cbc": NS["cbc"], "cac": NS["cac"]}

def _cbc(t): return f"{{{NS['cbc']}}}{t}"
def _cac(t): return f"{{{NS['cac']}}}{t}"
def _ubl(t): return f"{{{NS['ubl']}}}{t}"
def _cn(t): return f"{{{NS_CN}}}{t}"

def is_credit_note(inv) -> bool:
    return str(getattr(inv, "invoice_type_code", "380") or "380") == "381"
...
def _txt(parent, tag, text, ns="cbc"):
    ...
    # Namensfelder duerfen im Kundenstamm einen Zeilenumbruch tragen, damit die
    # Anschrift im PDF an der gewuenschten Stelle bricht. Im XML sind BT-27/BT-44
    # einzeilig — Umbrueche und Mehrfach-Leerzeichen werden hier eingeebnet.
...
def _build_address(parent, addr):
    ...
    # XRechnung / EN 16931 / UBL 2.1: cbc:StreetName + cbc:BuildingNumber
    # StreetName enthält Strasse + Hausnr (DIN 5008 lesbar);
    # BuildingNumber setzen wir zusätzlich für strukturierte Auswertung.
...
def _build_payment(root, inv, credit=False):
    pm = etree.SubElement(root, _cac("PaymentMeans"))
    _txt(pm, "PaymentMeansCode", inv.payment.means_code)
    # UBL-CreditNote kennt kein cbc:DueDate auf Wurzelebene — BT-9 liegt dort
    # in PaymentMeans/PaymentDueDate (direkt hinter dem PaymentMeansCode).
    if credit and inv.payment.due_date:
        _dt(pm, "PaymentDueDate", inv.payment.due_date)
    if inv.payment.iban:
...
def _build_monetary_total(root, inv):
    ...
    # BT-113: bereits geleistete Anzahlungen. Stand fruher fest auf 0,00 — die
    # Schlussrechnung zog die Abschlaege nur im PDF ab, die XRechnung forderte
    # den vollen Betrag (07.09.2026). BR-CO-16 verlangt:
    # PayableAmount = TaxInclusiveAmount - PrepaidAmount (+ Rundung).
    vorab = Decimal(str(getattr(inv, "prepaid_amount", 0) or 0))
    _amt(lmt, "PrepaidAmount", vorab, cur)
    _amt(lmt, "PayableAmount", inv.amount_due(), cur)

def _build_invoice_line(root, line, cur, credit=False):
    il = etree.SubElement(root, _cac("CreditNoteLine" if credit else "InvoiceLine"))
    _txt(il, "ID", line.line_id)
    if line.note: _txt(il, "Note", line.note)
    qty = etree.SubElement(il, _cbc("CreditedQuantity" if credit else "InvoicedQuantity"))
...
def generate_xrechnung(inv: Invoice) -> etree._Element:
    credit = is_credit_note(inv)
    if credit:
        root = etree.Element(_cn("CreditNote"), nsmap=NSMAP_CN)
    else:
    ...
    if inv.payment.due_date and not credit: _dt(root, "DueDate", inv.payment.due_date)
    _txt(root, "CreditNoteTypeCode" if credit else "InvoiceTypeCode", inv.invoice_type_code)
    ...
    _build_payment(root, inv, credit)
    ...
    for line in inv.lines: _build_invoice_line(root, line, inv.currency_code, credit)
    return root
