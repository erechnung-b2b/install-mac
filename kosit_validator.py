...
def _parse_kosit_report(report_bytes: bytes) -> KositResult:
    ...
    # 3b. VARL-Format (validator 1.5/1.6, varl/1): Befunde stehen NICHT als
    # svrl:failed-assert im Report, sondern als <rep:message level="error|
    # warning|information" code="BR-…">Text</rep:message>. Bis 03.09.2026 wurde
    # das übersehen → jeder KoSIT-Lauf meldete „gültig, 0 Fehler", auch bei
    # Zurückweisung (Storno-XML mit BR-CL-01).
    if not result.errors and not result.warnings:
        for m in _findall_local(root, "message"):
            level = (m.get("level", "") or "").lower()
            code = m.get("code", "") or ""
            msg = " ".join(m.itertext()).strip()
            entry = f"[{code}] {msg}" if code else msg
    ...
    # Zurückgewiesen, aber keine Fehlerzeile erkannt → das darf nicht „gültig"
    # aussehen; wenigstens einen Hinweis liefern.
    if not result.valid and not result.errors:
        result.errors.append("[KoSIT] Dokument zurückgewiesen (Details im Prüfbericht)")
