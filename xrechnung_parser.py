...
def _parse_payment(root) -> PaymentInfo:
    ...
    # Invoice: cbc:DueDate auf Wurzelebene; CreditNote: PaymentMeans/PaymentDueDate
    info.due_date = _find_date(root, "cbc:DueDate")
    if not info.due_date and pm is not None:
        info.due_date = _find_date(pm, "cbc:PaymentDueDate")
    return info
...
def _parse_line(line_el) -> InvoiceLine:
    # InvoiceLine traegt InvoicedQuantity, CreditNoteLine CreditedQuantity
    qty_el = line_el.find("cbc:InvoicedQuantity", NS)
    if qty_el is None:
        qty_el = line_el.find("cbc:CreditedQuantity", NS)
    ...
    line = InvoiceLine(
        ...
        quantity=_find_decimal(line_el, "cbc:InvoicedQuantity")
                 or _find_decimal(line_el, "cbc:CreditedQuantity"),
...
def parse_xrechnung(xml_bytes: bytes, source_file: str = "") -> Invoice:
    ...
    inv = Invoice(
        ...
        invoice_type_code=_find(root, "cbc:InvoiceTypeCode")
                          or _find(root, "cbc:CreditNoteTypeCode") or "380",
    ...
    for line_el in (root.findall("cac:InvoiceLine", NS)
                    or root.findall("cac:CreditNoteLine", NS)):
