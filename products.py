...
OFFER_BLOCK_KEYS = ("intro", "leistungsumfang", "lieferumfang",
                    "mitwirkung", "haftung", "agb")


def _normalize_offer_blocks(raw) -> dict:
    """Stellt sicher, dass offer_blocks ein dict mit allen 6 Keys ist."""
    if not isinstance(raw, dict):
        raw = {}
    return {k: str(raw.get(k, "")).strip() for k in OFFER_BLOCK_KEYS}
...
class ProductManager:
    ...
    def list_all(self, category: str = "", active_only: bool = False) -> list[dict]:
        products = self._load()
        for p in products:
            p.setdefault("offer_blocks", _normalize_offer_blocks(None))
        if category:
    ...
    def add(self, data: dict) -> dict:
        ...
        product = {
            "art_nr": art_nr,
            "name": name,
            # "freetext" = reine Textbaustein-Vorlage (ohne Preis), sonst normales Katalog-Produkt
            "type": "freetext" if data.get("type") == "freetext" else "product",
            ...
            "offer_blocks": _normalize_offer_blocks(data.get("offer_blocks")),
            # Vertrag separat: Dienstvertrag NICHT ins Angebot mergen, sondern
            # als eigener Mail-Anhang beifügbar (BAFA/KFW-Förderangebote).
            "contract_separate": bool(data.get("contract_separate", False)),
            "status": "AKTIV",
            "created_at": _ts(),
        }
    ...
    def update(self, art_nr: str, data: dict) -> Optional[dict]:
        ...
        for p in products:
            if p["art_nr"] == art_nr:
                for key in ("name", "type", "description", "category", "unit",
                    ...
                    if key in data:
                        ...
                        if key in ("ek_price", "vk_price", "tax_rate"):
                            ...
                            val = val.strip()
                        p[key] = val
                if "offer_blocks" in data:
                    p["offer_blocks"] = _normalize_offer_blocks(data.get("offer_blocks"))
                if "contract_separate" in data:
                    p["contract_separate"] = bool(data["contract_separate"])
                self._save(products)
                return p
        return None
