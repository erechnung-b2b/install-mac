...
def _find_logo_for_zugferd():
    """Sucht Logo für ZUGFeRD-Header — zuerst per UI hochgeladen, dann Default."""
    base = Path(__file__).resolve().parent
    # 1) Per UI hochgeladen via /api/logo → data/logo/
    upload_dir = base / "data" / "logo"
    if upload_dir.exists():
        for ext in (".png", ".jpg", ".jpeg", ".webp", ".gif"):
            p = upload_dir / f"logo{ext}"
            if p.exists():
                return str(p)
    # 2) Default-Logos im Projekt
...
def _build_visible_pdf(inv: Invoice) -> bytes:
    ...
    # Kopf: Logo + Verkäufer in zwei Spalten
    s = inv.seller
    seller_html = (
        f"<b>{s.name}</b><br/>"
    ...
    seller_para = Paragraph(seller_html, small)

    logo_path = _find_logo_for_zugferd()
    logo_cell = ""
    if logo_path:
        try:
            logo_cell = Image(logo_path, width=24 * mm, height=24 * mm,
                              kind="proportional")
        except Exception:
            logo_cell = ""

    header_tbl = Table(
        [[logo_cell, seller_para]],
