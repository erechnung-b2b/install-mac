"""
PDF-Import für Ausgangsrechnungen im Briefkopf-Format des Betreibers.
...
# ── Seller-Stammdaten ─────────────────────────────────────────────────
# Kommen aus data/mandant.json (kanonische Betreiberdaten). Bis 04.09.2026 standen
# sie hart hier im Code — und damit im Kunden-Repo. seller_overrides (aus den
# Mandanten-Einstellungen, webapp) gewinnen weiterhin über diese Vorbelegung.
_MANDANT_DATEI = Path(__file__).resolve().parent / "data" / "mandant.json"


def _seller_fallback() -> dict:
    m: dict = {}
    try:
        m = json.loads(_MANDANT_DATEI.read_text("utf-8"))
    except Exception:
        pass  # keine Datei → leere Felder, seller_overrides müssen liefern
    strasse = " ".join(x for x in (m.get("strasse"), m.get("hausnummer")) if x)
    land = (m.get("land") or "").strip()
    return dict(
        name=m.get("company_name") or "",
        street=strasse,
        post_code=m.get("plz") or "",
        city=m.get("ort") or "",
        country=land if len(land) == 2 else "DE",
        email=m.get("email") or "",
        telephone=m.get("telefon_international") or m.get("telefon") or "",
        iban=(m.get("iban") or "").replace(" ", ""),
        bic=(m.get("bic") or "").replace(" ", ""),
        bank_name=m.get("bank_name") or "",
        contact_name=m.get("person_name") or "",
    )
